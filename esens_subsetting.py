...
def ensSubset(wrfsensfile, analysis, memvalsfile, fullensnum, 
    ...
    var = {'300_hPa_GPH': 0, '500_hPa_GPH': 1, '700_hPa_GPH': 2, '850_hPa_GPH': 3,
       ...
       '925_hPa_V-Wind': 18, '850_hPa_Q': 19, 'SLP': 20, '2m_Temp': 21, '2m_Q': 22, '2m_Dewpt': 23, 
       '10m_U-Wind': 24, '10m_V-Wind': 25}
...
#        plt.savefig('sensfield_threshold')
#        plt.close()
#    except:
#        plt.close()
...
#        ax.add_feature(cfeat.COASTLINE, edgecolor='dimgray')
#        ax.set_extent([-125., -65., 18., 52.])
#        maxweighted = np.ma.amax(np.absolute(error_masked[0,4,:,:]))
#        print("Absolute Max Weighted for Member 5: ", maxweighted)
#        cflevs = np.linspace(0., 20, 101.)
#        #print(cflevs)
#        #print(np.shape(error_masked))
#        errorfield = ax.contourf(lons, lats, error_masked[0,4,:,:], cflevs, cmap='jet',
#                                     transform=ccrs.PlateCarree(), extend='max')
#        #errorfield.cmap.set_under(color=u'navy')
#        errorfield.cmap.set_over(color=u'darkred')
